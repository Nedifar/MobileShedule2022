    ...
    public partial class TeacherPage : ContentPage
    {
        ...
        private void PackSubscribe()
        {
            MessagingCenter.Subscribe<Page, ListPack>(
                ...
                (sender, str) =>
                {
                    ObservableCollection<string> vs = new ObservableCollection<string>(str.Teachers);
            ...
            while (true)
            {
                try
                {
                    if (pCabinet.SelectedIndex != -1)
                    {
                        ...
                        loading.IsAnimationPlaying = true;
                        cvSchedule.IsVisible = false;
                        var resGroup = await http.GetAsync(new Uri($"https://bsite.net/Abobus/api/lastdance/getteacher/{pCabinet.SelectedItem.ToString().Replace("★", "")}?date={DateSave.date.SelectedDate.ToString("MM.dd.yyyy")}"));
