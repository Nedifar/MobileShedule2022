...
namespace AgendaApp.Pages
{
    ...
    public partial class Settings : ContentPage
    {
        ...
        private async void load()
        {
            ...
            {
                var packLists = await http.GetAsync($"https://bsite.net/Abobus/api/lastdance/getdate/{DateTime.Now.ToString("MM.dd.yyyy")}");
                packLists.EnsureSuccessStatusCode();
                ListPack lp = packLists.Content.ReadAsAsync<ListPack>().Result;
                groupList = new ObservableCollection<string>(lp.Groups);
                groupList.Insert(0, "Не выбрано");
                pickGroup.ItemsSource = groupList;
                pickGroup.SelectedIndex = 0;
                teacherList = new ObservableCollection<string>(lp.Teachers);
                teacherList.Insert(0, "Не выбрано");
                pickTeacher.ItemsSource = teacherList;
                pickTeacher.SelectedIndex = 0;
                cabinetList = new ObservableCollection<string>(lp.Cabinets);
